...
const UNIT_ID  = 0; // FX5U uses unit ID 0
...
// Parse PLC address string to modbus address and type
function parseAddress(address) {
  const type = address[0].toUpperCase();
  const index = parseInt(address.slice(1), 10);

  if (Number.isNaN(index)) throw new Error(`Invalid address ${address}`);

  switch (type) {
    case 'M': return { type: 'coil', addr: 8192 + index }; // FX5U M coil offset
    case 'D': return { type: 'register', addr: index };
    default:  throw new Error(`Unsupported address ${address}`);
  }
}

// Read ON/OFF coil (M address)
exports.readOnOff = async (address) => {
  if (!isConnected) throw new Error('PLC not connected');

  const parsed = parseAddress(address);
  if (parsed.type !== 'coil') throw new Error(`Address ${address} is not ON/OFF type`);

  const resp = await client.readCoils(parsed.addr, 1);
  return resp.response.body.valuesAsArray[0]; // boolean
};

// Read numeric register (D address)
exports.readNumber = async (address) => {
  if (!isConnected) throw new Error('PLC not connected');

  const parsed = parseAddress(address);
  if (parsed.type !== 'register') throw new Error(`Address ${address} is not NUMBER type`);

  const resp = await client.readHoldingRegisters(parsed.addr, 1);
  return resp.response.body.valuesAsArray[0]; // INT16
};
...
exports.readValue = async (device) => {
  switch (device.data_display_type) {
    ...
    default:
      throw new Error(`Unsupported data_display_type: ${device.data_display_type}`);
  }
};

// Read any address (auto-detect coil or register)
exports.readAddress = async (address) => {
  ...
  if (parsed.type === 'coil') {
    const resp = await client.readCoils(parsed.addr, 1);
    return resp.response.body.valuesAsArray[0];
  }
